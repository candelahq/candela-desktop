...
void main() {
  group('CliStatusBanner', () {
    testWidgets(
      'returns SizedBox.shrink when installed, up to date, no error',
      (tester) async {
    ...
    testWidgets(
      ...
      (tester) async {
        await tester.pumpWidget(
          _wrap(
            const CliStatusBanner(isInstalled: false, isBrewAvailable: false),
          ),
        );
        ...
        if (Platform.isMacOS) {
          ...
          expect(
            find.textContaining('Candela CLI is required'),
            findsOneWidget,
          );
    ...
    testWidgets('shows Install button when !isInstalled && isBrewAvailable', (
      ...
      await tester.pumpWidget(
        _wrap(const CliStatusBanner(isInstalled: false, isBrewAvailable: true)),
      );
