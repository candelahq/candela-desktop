...
void main() {
  ...
  group('Step 1 — Details', () {
    ...
    testWidgets('Cloud mode: Next enabled after entering GCP Project', (
      ...
      await tester.enterText(find.byType(TextField).first, 'my-gcp-project');
      await tester.pumpAndSettle();
  ...
  group('Step 2 — Providers', () {
    testWidgets(
      'Solo mode shows only local providers (ollama, vllm, lmstudio)',
      (tester) async {
        await pumpScreen(tester);
        await tester.tap(find.text('Solo'));
        await tester.pumpAndSettle();
  ...
  group('Edge cases', () {
    ...
    testWidgets('shows subtitle descriptions for each mode card', (
      ...
      expect(find.text('Your own GCP project (Vertex AI)'), findsOneWidget);
      expect(
